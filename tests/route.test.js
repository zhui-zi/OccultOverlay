...
assert.equal(new Set(routes[1346].points.map((row) => `${row[0]}:${row[1]}:${row[2]}`)).size, 68,
  'North Horn route positions must be unique');
...
assert.equal(view.target.mapId, 1244, 'North Horn point 62 must retain its subterrane map assignment');
Route.next();
assert.equal(Route.view(position).target.routeNumber, 63);
assert.equal(Route.view(position).target.mapId, 1135, 'North Horn point 63 must return to the surface map');
