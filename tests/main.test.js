  ...
  OC: {
    ...
    Settings: {
      get(key) {
        ...
        if (key === 'potAlertSeconds') return potAlertSeconds;
      ...
      set(key, value) {
        ...
        if (key === 'potAlertSeconds') potAlertSeconds = value;
    ...
    i18n: {
      t(key) {
        return {
          ...
          pot_pre_alert: 'Pot spawning soon', minute_short: 'min', second_short: 'sec',
...
assert.match(treasureGuideRule[1], /width:\s*100%/, 'all guidance windows must share the stack width');
...
const guideStackRule = styles.match(/\.guide-stack\s*\{([^}]*)\}/);
assert.ok(guideStackRule, 'shared guidance stack style must exist');
assert.match(guideStackRule[1], /left:\s*8px/, 'guidance windows must stay against the left edge');
assert.match(guideStackRule[1], /width:\s*min\(248px/, 'guidance windows must match the fixed radar width');
...
assert.match(treasureGuideRule[1], /pointer-events:\s*none/,
  'guidance panels must keep the map clickable outside their action buttons');
const routeDockRule = styles.match(/\.route-dock\s*\{([^}]*)\}/);
assert.ok(routeDockRule, 'patrol guide dock style must exist');
assert.match(routeDockRule[1], /position:\s*absolute/);
assert.match(routeDockRule[1], /top:\s*100%/, 'patrol guidance must default below the available page');
assert.match(routeDockRule[1], /pointer-events:\s*none/, 'the patrol dock must not create a full-width click blocker');
assert.match(styles, /\.route-dock \.route-guide\s*\{[^}]*zoom:\s*var\(--ui-scale,\s*1\)/,
  'patrol content scaling must not move its dock above the map edge');
...
vm.runInNewContext(mapSource, layerSandbox, { filename: '../js/map.js' });
assert.deepEqual(JSON.parse(JSON.stringify(layerSandbox.OC.MAP_LAYERS.map(layer => [layer.key, layer.icon]))), [
  ['bronze', 'assets/map-icons/bronze-treasure.png'],
  ['silver', 'assets/map-icons/silver-treasure.png'],
  ['potN', 'assets/map-icons/magic-pot.png'],
  ['potS', 'assets/map-icons/magic-pot.png'],
  ['reroll', 'assets/map-icons/reroll.png'],
  ['bunny', 'assets/map-icons/carrot.png'],
  ['survey', 'assets/map-icons/survey-point.png'],
], 'rail controls must use the upstream Fast Switch game icons');
Array.from(new Set(layerSandbox.OC.MAP_LAYERS.map(layer => layer.icon))).forEach(iconPath => {
  const icon = fs.readFileSync(require.resolve('../' + iconPath));
...
assert.match(mainSource, /id="guide-stack"[\s\S]+?id="treasure-guide"[\s\S]+?id="route-dock"[\s\S]+?id="route-guide"/,
  'Magic Pot guidance must float while patrol guidance uses a separate bottom dock');
assert.doesNotMatch(mainSource, /includeTop\(document\.getElementById\('guide-stack'\)\)/,
  'Magic Pot guidance must float without participating in map placement');
assert.match(mainSource, /renderedMapSize[\s\S]+?routeDock\.style\.top/,
  'patrol guidance must follow the rendered map bottom without resizing the map');
...
assert.doesNotMatch(mainSource, /openPanel\s*===\s*'route'/, 'patrol guidance must not use the full popover lifecycle');
...
let guideStackHidden = true;
let guideStackHeight = 0;
const guideStackHost = {
  offsetTop: 52,
  get offsetHeight() { return guideStackHeight; },
  classList: { contains(name) { return name === 'hidden' && guideStackHidden; } },
  getBoundingClientRect() { return { top: 52, bottom: 52 + guideStackHeight, height: guideStackHeight }; },
};
let routeDockHidden = true;
let routeDockHeight = 0;
const routeDockHost = {
  style: {},
  get offsetHeight() { return routeDockHeight; },
  classList: { contains(name) { return name === 'hidden' && routeDockHidden; } },
  getBoundingClientRect() { return { top: 720 - routeDockHeight, bottom: 720, height: routeDockHeight }; },
};
const mapHost = {
  style: {},
  getBoundingClientRect() {
    const top = Number.parseFloat(this.style.top || '0') || 0;
    return { top, bottom: 720, width: 600, height: 720 - top };
  },
};
let noMap = false;
const appHost = {
...
sandbox.document.getElementById = id => ({
  ...
  'guide-stack': guideStackHost,
  'route-dock': routeDockHost,
...
routeDockHidden = false;
routeDockHeight = 208;
sandbox.OC.App.updateRadarPlacement();
assert.equal(mapHost.style.top, '46px', 'the patrol dock must not move the top edge of the map');
assert.equal(mapHost.style.bottom, '0px', 'the patrol guide must not resize the map');
assert.equal(routeDockHost.style.top, '654px', 'the patrol guide must begin below the rendered square map');
noMap = true;
sandbox.OC.Radar.targets = () => [];
sandbox.OC.App.updateRadar();
...
assert.equal(mapHost.style.bottom, '0px', 'a collapsed map must not retain the hidden patrol dock space');
sandbox.OC.Radar.targets = () => [radarTarget];
sandbox.OC.App.updateRadar();
assert.equal(radarClasses.has('hidden'), false, 'a pinned radar with targets must remain visible when the map is collapsed');
guideStackHidden = true;
guideStackHeight = 0;
radarPinned = false;
sandbox.OC.App.updateRadar();
...
assert.deepEqual(
  Array.from(sandbox.OC.State.highlights),
  [49, 2074],
  'two missed tracker refreshes must not remove active capsules',
);
sandbox.OC.App._highlightMissingSince = {
  49: Date.now() - 15001,
  2074: Date.now() - 15001,
...
sandbox.OC.State.highlights = [49];
sandbox.OC.App._highlightMissingSince = { 49: Date.now() - 7001 };
sandbox.OC.Overlay.memMeta = {
  49: { active: false, directorSeen: true, directorActive: false, source: 'CEDirector' },
};
sandbox.OC.App.refreshHighlights();
assert.deepEqual(
  Array.from(sandbox.OC.State.highlights),
  [],
  'an authoritative director end must retain the shorter removal grace',
);
...
potAlertSeconds = [600, 300, 180, 10];
...
sandbox.OC.App.checkPotPreAlert();
sandbox.Date = { now() { return (nextPotEpoch - 10) * 1000; } };
sandbox.OC.App.checkPotPreAlert();
...
sandbox.OC.App._potAlertedFor = null;
sandbox.Date = { now() { return (nextPotEpoch - 5) * 1000; } };
sandbox.OC.App.checkPotPreAlert();
sandbox.OC.App.checkPotPreAlert();
assert.deepEqual(potPreAlerts.map((entry) => entry.message), [
  'Pot spawning soon · 10 sec · N',
...
assert.match(settingsPop.innerHTML, /id="a-pot-times"[^>]+value="3m"[^>]+disabled/);
...
assert.equal(settingsControls['#a-pot-times'].disabled, true, 'pot reminder times must be disabled with the alert');
